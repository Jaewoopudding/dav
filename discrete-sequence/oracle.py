...
# Paths relative to this package (discrete-sequence/)
_ORACLE_DIR = os.path.dirname(os.path.abspath(__file__))
ATAC_CKPT_PATH = os.path.join(_ORACLE_DIR, 'artifacts', 'ATAC_oracle', 'binary_atac_cell_lines.ckpt')
...
def cal_atac_pred_new(seqs, model=None):
    ...
    """
    if model is None:
        model = LightningModel.load_from_checkpoint(ATAC_CKPT_PATH, map_location='cuda')
    # model.cuda()/
    model.eval()
...
class DNAQualityAssessor:
    def __init__(self):
        ...
        self.atac_model = LightningModel.load_from_checkpoint(ATAC_CKPT_PATH, map_location='cuda')
